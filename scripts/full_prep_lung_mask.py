...
logging.basicConfig(
    filename="/local_ssd/practical_wise24/lung_cancer/prasanga/logs/out.log",
    level=logging.INFO,
    format="%(asctime)s:%(levelname)s:%(message)s",
)

logging.basicConfig(
    filename="/local_ssd/practical_wise24/lung_cancer/prasanga/logs/err.log",
    level=logging.ERROR,
    format="%(asctime)s:%(levelname)s:%(message)s",
)
...
def process_mask(mask):
    """
    Process the input mask by applying convex hull and binary dilation operations.

    Args:
        mask (ndarray): The input mask array.

    Returns:
        ndarray: The processed mask array after applying convex hull and binary dilation operations.
    """
...
def lumTrans(img):
    """
    Applies lung windowing transformation to the input image.
...
def savenpy(id, filelist, prep_folder, data_path, use_existing=True):
    """
    Save the preprocessed lung mask as a numpy array.
    ...
    Args:
        ...
        use_existing (bool, optional): Flag to indicate whether to use existing preprocessed data if available. Defaults to True.
    """
...
def full_prep(data_path, prep_folder, n_worker=None, use_existing=True):
    """
    Preprocesses the data by saving the numpy arrays.
...
if __name__ == "__main__":
    orig_dir = "/local_ssd/practical_wise24/lung_cancer/prasanga/best_niftis"
    best_niftis = pd.read_csv("/local_ssd/practical_wise24/lung_cancer/prasanga/best_niftis.csv")
    ...
    for pid in pid_list:
        try:
            ...
            prep_folder = "/local_ssd/practical_wise24/lung_cancer/prasanga/segmented_lungs/" + pid
