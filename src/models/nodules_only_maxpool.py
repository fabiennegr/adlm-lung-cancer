...
def get_resnet3d():
    ...
    The model is initialized with pre-trained weights from ResNet-50.
...
class NodulesOnlyModule(LightningModule):
    ...
    def forward(self, nod1: torch.Tensor, nod2: torch.Tensor, nod3: torch.Tensor) -> torch.Tensor:
        ...
        # take the max-pool of enc_nod1, enc_nod2 and enc_nod3
        # enc_nod1 is of shape (batch_size, 576),
        # enc_nod2 is of shape (batch_size, 576),
        # enc_nod3 is of shape (batch_size, 576),
        # so the max-pool is of shape (batch_size, 576)

        print(f"enc_nod1 shape: {enc_nod1.shape}")
        max_pooled_nodules = torch.max(torch.stack((enc_nod1, enc_nod2, enc_nod3)), dim=0)[0]

        print(f"max_pool shape: {max_pooled_nodules.shape}")

        hidden = self.relu(self.fc1(max_pooled_nodules))
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        Args:
            batch (Tuple[torch.Tensor, torch.Tensor]): A batch of data (a tuple) containing the input tensor of images
                and target labels.
            batch_idx (int): The index of the current batch.
        """
        loss, preds, targets = self.model_step(batch)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        Computes and logs validation metrics such as accuracy, precision, recall, and F1 score.
        """
